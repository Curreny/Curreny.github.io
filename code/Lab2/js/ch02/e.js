...
var AppE = {
	...
	init: function() {
		this.canvas = document.getElementById("gl-canvas-e");
		...
		// 初始化Subdivision滑块
		const sliderE = document.getElementById("subdivision-slider-e");
		const sliderValueE = document.getElementById("slider-value-e");
		sliderE.value = this.numTimesToSubdivide;
		sliderE.oninput = () => {
			this.numTimesToSubdivide = parseInt(sliderE.value);
			sliderValueE.textContent = this.numTimesToSubdivide;
			this.points = [];
			this.renderGasket();
		};

		// 初始化Theta滑块
		const thetaSliderE = document.getElementById("subdivision-slider-theta-e");
		const thetaValueE = document.getElementById("slider-value-theta-e");
		thetaSliderE.value = this.theta;
		thetaSliderE.oninput = () => {
			this.theta = parseInt(thetaSliderE.value);
			thetaValueE.textContent = this.theta;
			this.points = [];
			this.renderGasket();
		};
	...
	renderGasket: function() {
		...
		var program = initShaders(this.gl, "vertex-shader-e", "fragment-shader-e");
