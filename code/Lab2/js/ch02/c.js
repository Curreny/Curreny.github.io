...
var AppC = {
	...
	init: function() {
		this.canvas = document.getElementById("gl-canvas-c");
		...
		// 初始化滑块控件并设置事件监听器
		const sliderC = document.getElementById("subdivision-slider-c");
		const sliderValueC = document.getElementById("slider-value-c");
		sliderC.value = this.numTimesToSubdivide;
		sliderValueC.innerHTML = this.numTimesToSubdivide;

		sliderC.oninput = () => {
			this.numTimesToSubdivide = parseInt(sliderC.value);
			sliderValueC.innerHTML = this.numTimesToSubdivide;
			this.points = [];
			this.renderGasket(); // 重新初始化三角形
		};
	...
	renderGasket: function() {
		...
		var program = initShaders(this.gl, "vertex-shader-c", "fragment-shader-c");
